...
class CartService
{
    ...
    public function payload(Cart $cart): array { $cart = $this->load($cart); $items = $cart->items->map(fn (CartItem $item) => ['id' => $item->id, 'book_id' => $item->book_id, 'title' => $item->book->title, 'quantity' => $item->quantity, 'unit_price' => $item->book->final_price, 'original_price' => $item->book->final_price < $item->book->price ? $item->book->price : null, 'subtotal' => round((float) $item->book->final_price * $item->quantity, 2)]); return ['id' => $cart->id, 'guest_token' => $cart->guest_token, 'items' => $items, 'subtotal' => $items->sum(fn ($item) => (float) $item['subtotal'])]; }
