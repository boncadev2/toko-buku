...
export default function CheckoutPage() {
  ...
  return <main className="min-h-screen bg-[#f4f8ff] text-slate-900">
    ...
    <form onSubmit={submit} className="mx-auto grid max-w-5xl gap-6 px-5 py-9 lg:grid-cols-[1fr_320px]">
      ...
      <aside className="h-fit rounded-3xl bg-white p-6 shadow-sm"><h2 className="font-black">Konfirmasi checkout</h2><p className="mt-2 text-sm leading-6 text-slate-500">Ongkir diperbarui sesuai lokasi yang dipilih.</p><div className="mt-5 space-y-3 rounded-xl bg-blue-50 p-4 text-sm"><div className="flex justify-between"><span>Subtotal belanja</span><b>{cart ? money(cart.subtotal) : "Memuat…"}</b></div><div className="flex justify-between"><span>Ongkir</span><b>{selectedQuote ? money(selectedQuote.cost ?? selectedQuote.price ?? selectedQuote.final_price) : "—"}</b></div><div className="flex justify-between border-t border-blue-200 pt-3 text-base"><b>Total belanja</b><b className="text-blue-700">{cart ? money(Number(cart.subtotal || 0) + Number(selectedQuote?.cost ?? selectedQuote?.price ?? selectedQuote?.final_price ?? 0)) : "—"}</b></div></div><button className="mt-6 w-full rounded-xl bg-blue-700 py-3 font-bold text-white">Buat Pesanan</button>{message && <p className="mt-4 text-sm font-semibold text-blue-700">{message}</p>}</aside>
    </form>
  </main>;
}
