...
<div className="hidden items-center gap-3 sm:flex"><a href="/keranjang" className="relative grid h-10 w-10 place-items-center rounded-full border border-blue-200 bg-white text-lg text-blue-700 shadow-sm transition hover:bg-blue-50" aria-label="Keranjang belanja" title="Keranjang belanja">🛒<span className="absolute -right-1 -top-1 grid h-4 min-w-4 place-items-center rounded-full bg-blue-700 px-1 text-[9px] font-black text-white">{cartCount}</span></a>{user ? <AccountMenu user={user} onLogout={logout} /> : <><button className="text-sm font-bold text-stone-700">Masuk</button><button className="rounded-full bg-stone-950 px-4 py-2 text-sm font-bold text-white">Daftar</button></>}</div>
