...
export default function MarketplaceFloatingButtons({ cartCount = 0, showCart = false }) {
  ...
  return <aside className="fixed bottom-5 right-4 z-40 flex flex-col items-end gap-3 sm:bottom-7 sm:right-7" aria-label="Pesan melalui marketplace">
    ...
    {showCart && <a href="/keranjang" className="relative sm:hidden grid h-12 w-12 place-items-center rounded-full border border-slate-200 bg-white text-xl text-blue-700 shadow-lg transition hover:scale-110" aria-label="Keranjang belanja" title="Keranjang belanja">🛒<span className="absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-red-500 px-1 text-[10px] font-black text-white">{cartCount}</span></a>}
