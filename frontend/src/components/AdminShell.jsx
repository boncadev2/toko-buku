...
export default function AdminShell({ title, description, children, headerRight }) {
  ...
  useEffect(() => { api("/settings").then((payload) => ((d) => { localStorage.setItem("app_settings", JSON.stringify(typeof d === "function" ? d(site) : d)); setSite(d); })((old) => ({ ...old, ...payload.data }))).catch(() => null); }, []);
  ...
  return <main className="flex min-h-screen flex-col bg-[#f4f7fb] text-slate-800">
    <header className="sticky top-0 z-40 border-b border-blue-100 bg-white/95 backdrop-blur">
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-4">
        <Link href="/" className="flex items-center gap-3">{site.app_logo ? <img src={site.app_logo} alt={site.app_name} className="h-10 w-10 rounded-xl object-contain"/> : <span className="grid h-10 w-10 place-items-center rounded-xl bg-blue-700 text-lg text-white">{(site.app_name || " ")[0].toUpperCase()}</span>}<span><b className="block text-lg leading-none text-blue-950">{site.app_name || ""}</b><small className="text-[10px] font-bold uppercase tracking-widest text-blue-500">Toko buku online</small></span></Link>
      ...
      {open && <nav className="grid gap-1 border-t bg-white p-3 lg:hidden">{menu.map(([href, icon, label]) => <Link key={href} href={href} className={`rounded-xl px-4 py-3 text-sm font-bold ${pathname === href ? "bg-blue-700 text-white" : "text-slate-600"}`}>{icon} {label}</Link>)}<button onClick={logout} className="rounded-xl px-4 py-3 text-left text-sm font-bold text-red-600">↪ Keluar</button></nav>}
    </header>
    ...
    <footer className="mt-8 border-t border-blue-100 bg-white">
      <div className="mx-auto flex w-full max-w-7xl flex-wrap items-center justify-between gap-3 px-5 py-6 text-sm text-slate-500">
        <span>© 2026 {site.app_name || ""}. Semua hak dilindungi.</span>
        <div className="flex gap-4"><Link href="/info/tentang" className="hover:text-blue-700">Tentang</Link><Link href="/info/kontak" className="hover:text-blue-700">Bantuan</Link><Link href="/info/pengiriman" className="hover:text-blue-700">Pengiriman</Link></div>
      </div>
    </footer>
  </main>;
}
